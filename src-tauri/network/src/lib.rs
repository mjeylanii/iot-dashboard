...
pub fn send_arp_request(
    ...
    let mut arp_packet =
        MutableArpPacket::new(&mut arp_buffer).context("Can't build arp request")?;
...
pub fn find_source_ip(network_interface: &NetworkInterface) -> Result<Ipv4Addr> {
    ...
    match potential_network.map(|network| network.ip()) {
        ...
        _ => Err(eyre!("Expected ipv4 on interface")),
    }
}
...
) -> Result<(ResponseSummary, Vec<TargetDetails>)> {
...
pub fn scan(interface: &SelectInterface) -> Result<Vec<Host>> {
    ...
    let selected = interfaces
        ...
        .ok_or_else(|| eyre!("Interface not found"))?;

    let ip_networks: Vec<&ipnetwork::IpNetwork> = selected
        .ips
        .iter()
    ...
    let channel = pnet_datalink::channel(selected, channel_config).expect(
        "Failed to create a datalink channel. This could be due to insufficient permissions.",
    );
    ...
    let arp_responses =
        thread::spawn(move || receive_arp_responses(&mut rx, cloned_timed_out, &mut vendor_list));
    ...
    let mut found_hosts: Vec<Host> = target_details
        .iter()
        .map(|t| Host {
            host: t.ipv4.to_string(),
            hostname: t.hostname.clone().unwrap_or_default(),
            mac: t.mac.to_string(),
            vendor: t.vendor.clone().unwrap_or_default(),
        })
        .collect();
...
mod tests {
    ...
    fn should_iterate_over_multiple_networks() {
        ...
        assert_eq!(
            iterator.next(),
            Some(IpAddr::V4(Ipv4Addr::new(10, 10, 20, 20)))
        );
        assert_eq!(iterator.next(), None);
    }
